...
const FormCleaner = ({ service }: any) => {
  [service.queryParmas, service.setQueryParmas] = React.useState(service.queryParmas);
  return React.useMemo(() => {
    const toDelete = service.queryParmas && Object.entries(service.queryParmas).filter(([_, value]: any) => value?.value);
    console.log({ toDelete });
    return toDelete && toDelete.length > 0 ? (
      <div className="form-cleaner">
        {toDelete.map(([_, _queryProp]: any) => (
          <p
            key={_}
            onClick={() => {
