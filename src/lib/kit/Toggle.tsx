...
const Toggle = ({ className = "", storageKey, value: defaultValue = "false", title, onChange, id, init }) =>
  React.useMemo(() => {
    ...
    return (
      <div className={`kit-toggled-container ${className}`}>
        <p className="font-bold text-s px-s">{title}</p>
        <p
...
//             <p className="font-bold text-s px-s">{prop.title}</p>
//             <p
//                 ref={(ref) => {
